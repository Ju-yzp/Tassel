...
TEST(ReprojectionTest, SplitTransformMatchesComposedTransform) {
    FrameState host;
    host.pos_w_i = Eigen::Vector3d(0.2, -0.1, 0.3);
    host.vel_w = Eigen::Vector3d(0.4, 0.1, -0.2);
    host.imu_gyro = Eigen::Vector3d(0.1, -0.2, 0.3);
    host.imu_acc = Eigen::Vector3d(0.2, 0.3, 9.7);
    FrameState target;
    target.pos_w_i = Eigen::Vector3d(0.4, 0.0, 0.2);
    target.vel_w = Eigen::Vector3d(0.3, -0.1, 0.0);
    target.imu_gyro = Eigen::Vector3d(-0.1, 0.2, 0.1);
    target.imu_acc = Eigen::Vector3d(0.1, -0.2, 9.6);
...
TEST(FeatureManagerTest, TransfersDepthWhenOldestHostLeaves) {
    ...
    state.frames[0].pos_w_i = Eigen::Vector3d::Zero();
    state.frames[1].pos_w_i = Eigen::Vector3d(0.1, 0.0, 0.0);
    state.frames[2].pos_w_i = Eigen::Vector3d(0.2, 0.0, 0.0);
...
TEST(FeatureManagerTest, DepthTransferIncludesGravityAndFrameDelay) {
    ...
    state.time_delay = 0.1;
    state.frames[0].image_sync_delay = 0.0;
    state.frames[1].image_sync_delay = 0.05;
    Feature feature(0, 2);
...
TEST(FeatureManagerTest, RemovingMiddleFrameCompactsFeatureHostIndex) {
    auto fm = manager();
    State state(4);
    state.latest_active_frame_index = 3;
    Feature feature(2, 2);
    feature.estimated_depth = 2.0;
    feature.observations = {observation(), observation(0.1)};
    fm.features().emplace(1, std::move(feature));
...
TEST(FeatureManagerTest, TriangulatesDepthBeyondExportLimit) {
    ...
    state.frames[2].pos_w_i = Eigen::Vector3d(2.0, 0.0, 0.0);
...
TEST(FeatureManagerTest, TriangulationUsesThirdOrderFrameCompensation) {
    State state(3);
    state.latest_active_frame_index = 2;
    state.time_delay = 0.12;
    state.frames[0].pos_w_i = Eigen::Vector3d(0.0, 0.0, 0.0);
    state.frames[1].pos_w_i = Eigen::Vector3d(0.35, 0.02, -0.01);
    state.frames[2].pos_w_i = Eigen::Vector3d(0.72, -0.01, 0.03);
    for (int i = 0; i < 3; ++i) {
        auto& frame = state.frames[i];
        frame.vel_w = Eigen::Vector3d(0.4 + 0.1 * i, -0.15, 0.08);
        frame.imu_gyro = Eigen::Vector3d(0.7, -0.4 + 0.1 * i, 0.5);
        frame.imu_acc = Eigen::Vector3d(1.2, -0.6, 9.3);
        frame.gyro_bias = Eigen::Vector3d(0.02, -0.01, 0.03);
        frame.accel_bias = Eigen::Vector3d(0.08, -0.04, 0.05);
        frame.image_sync_delay = 0.01 + 0.02 * i;
    }
    ...
    Feature feature(0, 3);
    feature.observations.push_back(observation(host_uv.x(), state.frames[0].image_sync_delay));
    feature.observations.back().uv = host_uv;
    for (int target_index = 1; target_index <= 2; ++target_index) {
        Eigen::Vector3d target_point;
        ASSERT_TRUE(reprojectToTargetCamera(
            state.frames[0], state.frames[target_index], host_uv, depth,
            state.frames[0].image_sync_delay, state.frames[target_index].image_sync_delay,
            state.time_delay, ric, tic, target_point));
        ...
        target_observation.sync_delay = state.frames[target_index].image_sync_delay;
...
TEST(FeatureManagerTest, ExportsValidLandmarksForRequestedHostAsIndependentValues) {
    ...
    State state(3);
    state.latest_active_frame_index = 2;
    state.frames[1].rot_w_i =
        Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitZ()).toRotationMatrix();
    state.frames[1].pos_w_i = Eigen::Vector3d(4.0, 5.0, 6.0);
    Feature valid(1, 2);
...
TEST(FeatureManagerTest, ExportsObservedHostDepthAsWorldLandmark) {
    ...
    State state(2);
    state.latest_active_frame_index = 1;
    state.frames[0].rot_w_i =
        Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitZ()).toRotationMatrix();
    state.frames[0].pos_w_i = Eigen::Vector3d(4.0, 5.0, 6.0);
    Feature feature(0, 2);
    feature.estimated_depth = 2.0;
