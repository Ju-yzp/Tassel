...
bool VisualFactor::Evaluate(
    ...
    Eigen::Vector3d pi_in_G =
        R_i * A_i * pi_in_I + P_i + V_i * dt + 0.5 * R_i * (a_i - ba_i) * dt * dt;
    Eigen::Vector3d pj_in_I =
        A_j * R_j.transpose() * (pi_in_G - (P_j + V_j * dt + 0.5 * R_j * (a_j - ba_j) * dt * dt));
    ...
    if (jacobians) {
        ...
        if (jacobians[0]) {
            ...
            jacobian_pose_i.block<2, 3>(0, 3) = -reduce * ric.transpose() * A_j * R_j.transpose() *
                                                ...
                                                (Sophus::SO3d::hat(A_i * pi_in_I) +
                                                 0.5 * Sophus::SO3d::hat((a_i - ba_i) * dt * dt));
        }
        ...
        if (jacobians[1]) {
            ...
            jacobian_pose_j.block<2, 3>(0, 3) =
                ...
                Sophus::SO3d::hat(R_j.transpose() * (pi_in_G - (P_j + V_j * dt)));
        }
