...
bool SE3RightManifold::Plus(const double* x, const double* delta, double* x_plus_delta) const {
    Eigen::Vector3d phi(x[0], x[1], x[2]);
    Eigen::Vector3d P(x[3], x[4], x[5]);
    Eigen::Vector3d dphi(delta[0], delta[1], delta[2]);
    Eigen::Vector3d dP(delta[3], delta[4], delta[5]);
    ...
    for (int i = 0; i < 3; ++i) {
        x_plus_delta[i] = phi_new(i);
    }
    for (int i = 0; i < 3; ++i) {
        x_plus_delta[3 + i] = P_new(i);
    }
    return true;
}
...
bool SE3RightManifold::PlusJacobian(const double* x, double* jacobian) const {
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            jacobian[i * 6 + j] = Jr_inv(i, j);
        }
    }
    for (int i = 0; i < 3; ++i) {
        jacobian[(3 + i) * 6 + (3 + i)] = 1.0;
    }
    return true;
}

bool SE3RightManifold::Minus(const double* y, const double* x, double* y_minus_x) const {
    Eigen::Vector3d phi_y(y[0], y[1], y[2]);
    Eigen::Vector3d P_y(y[3], y[4], y[5]);
    Eigen::Vector3d phi_x(x[0], x[1], x[2]);
    Eigen::Vector3d P_x(x[3], x[4], x[5]);
