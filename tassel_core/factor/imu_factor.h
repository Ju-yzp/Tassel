...
public:
    ...
    bool Evaluate(
        ...
        Eigen::Vector3d phi_j(parameters[2][0], parameters[2][1], parameters[2][2]);
        ...
        if (jacobians) {
            if (jacobians[0]) {
                ...
                jacobian_pose_i.template block<3, 3>(0, 0) = Sophus::SO3d::hat(
                    R_i.transpose() *
                    (0.5 * tassel_utils::G * sum_dt * sum_dt + P_j - P_i - V_i * sum_dt));
                jacobian_pose_i.template block<3, 3>(0, 3) = -R_i.transpose();
                jacobian_pose_i.template block<3, 3>(3, 0) = Jr_inv * R_j.transpose() * R_i;
                jacobian_pose_i.template block<3, 3>(6, 0) =
            ...
            if (jacobians[2]) {
                ...
                jacobian_pose_j.block<3, 3>(0, 3) = R_i.transpose();
                jacobian_pose_j.block<3, 3>(3, 0) = Jr_inv;
