...
void FeatureManager::triangulate(
    ...
    bool mono_triangulate =
        frame_count > 0 &&
        (state->Ps[frame_count] - state->Ps[frame_count - 1]).norm() > min_translation_;
...
void FeatureManager::initPoseByPNP(
    ...
    if (!state) {
        throw std::runtime_error(
            std::string("[") + __FUNCTION__ +
            "] Input 'state' pointer is null. Pre-condition failed.");
    }
    ...
    for (const auto& [id, feature] : features_) {
        size_t start_frame_id = feature.start_frame_id;
        size_t observation_num = feature.observations.size();
        ...
        if (frame_count + 1 == observation_num && depth != INVALID_DEPTH) {
            ...
            if (object_pts.size() >= max_pnp_needed_num_) {
                break;
            }
        }
    }

    if (object_pts.size() < min_pnp_pt_num_) {
        return;
    }
    ...
    if (success) {
        ...
        state->Rs[frame_count] = guess_R * ric.transpose();
        state->Ps[frame_count] = guess_P - state->Rs[frame_count] * tic;
    }
}
...
void FeatureManager::removeOutliers(
    ...
    for (auto& [id, feature] : features_) {
        ...
        if (depth == INVALID_DEPTH || observations.size() < tracked_times_thres_) {
            continue;
        }

        int start_frame_id = feature.start_frame_id;
        const Eigen::Matrix3d& R_i = state->Rs[start_frame_id];
        const Eigen::Vector3d& P_i = state->Ps[start_frame_id];
        ...
        for (size_t k = 1; k < observations.size(); ++k) {
            ...
            const Eigen::Matrix3d& R_j = state->Rs[j];
            const Eigen::Vector3d& P_j = state->Ps[j];
...
std::vector<std::pair<int, const Feature*>> FeatureManager::collectMarginalizationFeatures() const {
    ...
    for (const auto& [id, feature] : features_) {
        ...
        if (feature.observations.size() < tracked_times_thres_) continue;
        result.emplace_back(id, &feature);
    }
    return result;
}

std::vector<Eigen::Vector3d> FeatureManager::get_pointcloud(
    std::shared_ptr<State> state, const Eigen::Matrix3d& ric, const Eigen::Vector3d& tic) {
    ...
    for (auto& [id, feature] : features_) {
        ...
        const Eigen::Matrix3d& R_i = state->Rs[start_frame_id];
        const Eigen::Vector3d& P_i = state->Ps[start_frame_id];
        Eigen::Vector3d pi_in_C = depth * feature.observations[0].uv;
        Eigen::Vector3d pi_in_I = ric * pi_in_C + tic;
        Eigen::Vector3d pi_in_W = R_i * pi_in_I + P_i;
        pointcloud.push_back(pi_in_W);
    }
    return pointcloud;
}
