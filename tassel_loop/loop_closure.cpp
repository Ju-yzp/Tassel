...
KeyframeId LoopClosure::submitKeyframe(LoopKeyframeTransaction transaction) {
    ...
    return submit(std::move(transaction), true);
}
...
KeyframeId LoopClosure::submit(Transaction transaction, bool assign_keyframe_id) {
    ...
    KeyframeId keyframe_id = kInvalidKeyframeId;
    {
        ...
        if (assign_keyframe_id) {
            keyframe_id = next_keyframe_id_++;
            std::get<LoopKeyframeTransaction>(transaction).keyframe_id = keyframe_id;
        }
...
void LoopClosure::process(const LoopPoseTransaction& transaction) {
    ...
    LoopClosureResult result = snapshot(LoopEvent::GlobalPoseUpdated);
...
void LoopClosure::process(const LoopKeyframeTransaction& transaction) {
    if (transaction.source_frame_id == tassel_utils::kInvalidFrameId ||
        transaction.keyframe_id == kInvalidKeyframeId || graph_.contains(transaction.keyframe_id)) {
        throw std::invalid_argument("Loop keyframe is invalid or duplicated");
    }
    keyframe_sources_.emplace(transaction.keyframe_id, transaction.source_frame_id);
    keyframe_local_poses_.emplace(transaction.keyframe_id, transaction.local_T_imu);
    keyframe_images_.emplace(transaction.keyframe_id, transaction.image);
    const Sophus::SE3d local_T_camera = transaction.local_T_imu * options_.imu_T_camera;
    graph_.addKeyframe(
        transaction.keyframe_id, local_T_camera.rotationMatrix(), local_T_camera.translation());
    LoopClosureResult result = snapshot(LoopEvent::GraphUpdated);
    result.current_frame_id = transaction.source_frame_id;
    publish(std::move(result));
}

void LoopClosure::process(const LoopLandmarkTransaction& transaction) {
    const auto image = keyframe_images_.find(transaction.keyframe_id);
    if (image == keyframe_images_.end() || transaction.landmarks.empty()) {
        LoopClosureResult result = snapshot(LoopEvent::LoopRejected);
        const auto source_frame = keyframe_sources_.find(transaction.keyframe_id);
        if (source_frame != keyframe_sources_.end()) {
            result.current_frame_id = source_frame->second;
        }
    ...
    const LoopQuery query = database_.addKeyframe(transaction.keyframe_id, image->second);
    database_.attachLandmarks(transaction.keyframe_id, transaction.landmarks);
    keyframe_images_.erase(image);
    verify(query);
}
...
void LoopClosure::verify(const LoopQuery& query) {
    ...
    LoopClosureResult result =
        snapshot(accepted ? LoopEvent::LoopAccepted : LoopEvent::LoopRejected);
    result.current_frame_id = keyframe_sources_.at(best_matches.current_frame_id);
    result.candidate_frame_id = keyframe_sources_.at(best_matches.candidate_frame_id);
    ...
    const auto local_pose = keyframe_local_poses_.find(best_matches.current_frame_id);
    if (graph_pose && local_pose != keyframe_local_poses_.end()) {
...
LoopClosureResult LoopClosure::snapshot(LoopEvent event) const {
    ...
    if (event != LoopEvent::GlobalPoseUpdated) {
        result.graph_poses = graph_.poses();
    }
    if (event == LoopEvent::LoopAccepted) {
        std::vector<TimedPose> local_keyframes;
        std::vector<TimedPose> global_keyframes;
        for (const auto& [keyframe_id, graph_pose] : result.graph_poses) {
            const auto local_pose = keyframe_local_poses_.find(keyframe_id);
            const auto source_frame = keyframe_sources_.find(keyframe_id);
            if (local_pose != keyframe_local_poses_.end() &&
                source_frame != keyframe_sources_.end()) {
                local_keyframes.push_back(
                    {source_frame->second, local_pose->second * options_.imu_T_camera});
                global_keyframes.push_back(
                    {source_frame->second,
                     Sophus::SE3d(graph_pose.world_R_camera, graph_pose.world_t_camera)});
            }
        }
